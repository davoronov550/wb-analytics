...
class TestProductCreate:
    ...
    def test_nonpositive_wb_id_raises(self) -> None:
        with pytest.raises(ValueError):
            _make(wb_id=0)

    def test_clamps_sale_price_above_price(self) -> None:
        p = _make(price=Money(Decimal("50.00")), sale_price=Money(Decimal("80.00")))
        assert p.sale_price == Money(Decimal("50.00"))

    def test_keeps_sale_price_at_or_below_price(self) -> None:
...
class TestProductRehydrate:
    def test_rehydrate_builds_exact_values(self) -> None:
        p = Product.rehydrate(
            wb_id=5,
            name="A",
