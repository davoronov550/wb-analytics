...
class FakeParseJobRepository:
    ...
    def get(self, task_id: str) -> ParseJob | None:  # pragma: no cover - not used here
        raise NotImplementedError

    def mark_running(self, task_id: str) -> None:  # pragma: no cover - not used here
        raise NotImplementedError
...
class TestFirstRequestStartsARun:
    def test_creates_a_pending_job_and_enqueues_it(self) -> None:
        repository, queue = FakeParseJobRepository(), FakeQueue()

        job = _make(repository, queue).execute("наушники")

        assert job.status == "pending"
        assert repository.created == ["наушники"]
        assert len(queue.enqueued) == 1

    def test_the_payload_carries_what_the_worker_needs(self) -> None:
        """The worker receives only this dict — a missing key surfaces as a
        failed run, not as a validation error here."""
        repository, queue = FakeParseJobRepository(), FakeQueue()

        job = _make(repository, queue).execute("наушники", max_pages=3)
    ...
    def test_max_pages_is_passed_through_as_none_when_omitted(self) -> None:
        ...
        _make(repository, queue).execute("наушники")
...
class TestASecondRequestDoesNotStartAnother:
    ...
    def test_returns_the_job_already_in_flight(self) -> None:
    ...
    def test_creates_nothing_and_enqueues_nothing(self) -> None:
