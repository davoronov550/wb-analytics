...
def parse_deps(raw: str) -> list[str]:
    """Handle '—', 'T010', 'T013, T017' and ranges like 'T010-T022'."""
    ...
    for chunk in raw.split(","):
        ...
        if rng:
            ...
            continue
        single = re.fullmatch(r"T\d{3}", chunk)
        if single:
            out.append(chunk)
    return out
...
def mermaid(tasks: list[Task], on_path: set[str]) -> str:
...
def render(tasks: list[Task]) -> str:
    ...
    out = [
        ...
        "",
        f"Готово **{done} из {total}** задач фаз 0-1.",
        "",
        "## Как читать",
        "",
        "| Цвет | Значение |",
