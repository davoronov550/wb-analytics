...
Both API prefixes are covered: `/v1/` is an alias of `/api/` (see
`config.urls`), so a client on the new prefix must not get a different
error shape from one on the old.
