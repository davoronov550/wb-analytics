...
### Гарантии доставки: где сообщение теряется и что этому мешает

Разобрано по точкам отказа, потому что «Kafka надёжна» — не утверждение, а
набор настроек, каждая из которых по умолчанию небезопасна.

| Точка отказа | Что защищает |
|---|---|
| Сервис упал до записи в БД | Работы не было — терять нечего |
| Упал между коммитом и публикацией | **Transactional outbox** |
| Kafka недоступна при публикации | Реле повторит из таблицы |
| Лидер партиции упал до репликации | **`acks=all`** + `min.insync.replicas=2` |
| Продюсер повторил после потерянного ack | **`enable.idempotence=true`** |
| Консьюмер упал до обработки | Офсет не сдвинут — повторная доставка |
| Обработчик упал | Офсет не сдвинут, **`event_id` помечается после успеха** |
| Сообщение-«отравитель» | **DLQ** после `max_attempts` |
| Новая консьюмер-группа | **`auto_offset_reset=earliest`** |

**Умолчания клиента опасны, поэтому настройки не документируются, а
задаются фабриками** `producer_config()` и `consumer_config()` в
`libs/platform/kafka.py`. Проверено на aiokafka 0.14:

- `enable_idempotence=False` — повтор продюсера создаёт дубль;
- `enable_auto_commit=True` — офсет двигается по таймеру, то есть падение
  обработчика теряет сообщение молча;
- `auto_offset_reset='latest'` — новая группа **пропускает всё накопленное**.
  Для бэкфилла аналитики это полная и бесшумная потеря данных.

**Настройки топиков в продакшене:**
...
`min.insync.replicas=2` при `replication.factor=3` — то, что придаёт `acks=all`
смысл: запись подтверждается, только когда её приняли минимум две реплики.
`unclean.leader.election=false` запрещает выбирать лидером отставшую реплику,
то есть менять потерю доступности на потерю данных.

**Дедупликация — оптимизация, а не гарантия.** `event_id` помечается
обработанным **после** успеха обработчика. Падение в окне между завершением
обработчика и записью метки приведёт к повторному выполнению. Закрыть это окно
можно только записав метку в одной транзакции с данными обработчика, а это
доступно лишь самому обработчику. Поэтому корректность обеспечивается на уровне
данных — `INSERT … ON CONFLICT DO NOTHING` в PostgreSQL, `ReplacingMergeTree` в
ClickHouse, — а дедуп лишь экономит лишнюю работу.
...
### Соответствие агрегатов — проверено на одинаковых данных

Первая редакция этого раздела содержала три ошибки, каждая из которых дала бы
**молча неверную аналитику**, а не отказ. Ниже — версия, сверенная построчно с
PostgreSQL на 500 одинаковых строк.

| Метрика | PostgreSQL сейчас | ClickHouse | Совпало |
|---|---|---|---|
| Количество | `count(*)` | `count()` | да |
| Средняя цена | `Avg` | `avg` | до 4 знаков |
| **Медиана** | `PERCENTILE_CONT(0.5)` | **`quantileExactInclusive(0.5)(toFloat64(x))`** | да |
| Стандартное отклонение | `StdDev` (Django → `stddev_pop`) | `stddevPop` | до 4 знаков |
| Средняя скидка | `Avg(price - sale_price)` | `avg(price - sale_price)` | до 4 знаков |
| Доля скидок | `Count(filter=…)` | `countIf(sale_price < price) / count()` | да |
| **Топ по отзывам** | `ORDER BY … DESC LIMIT 5` | **`ORDER BY … DESC LIMIT 5`** | да |
| **Гистограмма** | равноширинная, на клиенте | **`widthBucket(x, lo, hi, n)`** | корзина в корзину |

**Три отвергнутые функции и почему:**

1. **`quantileExact` — не медиана PostgreSQL.** Он возвращает существующий
   элемент выборки, а `PERCENTILE_CONT` **интерполирует** между двумя средними
   при чётном числе строк. На контрольных данных: 2868 против 2866. Верен
   `quantileExactInclusive`, но он **не принимает `Decimal`** — нужен
   `toFloat64`. На нечётном числе строк оба варианта совпадают, поэтому
   выборочная проверка расхождения не покажет.
2. **`topK` — не топ-N по значению.** Он возвращает самые **частые** значения:
   на тех же данных `[179, 483, 653, 670, 8]` вместо `[892, 891, 890, 889, 888]`.
   Функции с похожим именем и совершенно другой семантикой.
3. **`histogram(N)` — адаптивные корзины, а не равноширинные.** Ширины на
   контрольных данных: `[400.5, 452.9, 554.3, 543.7, …]`. Фронтенд
   ([lib/histogram.ts](../../frontend/src/lib/histogram.ts)) строит равные по
   ширине, и паритет требует именно их. `widthBucket` воспроизводит его точно,
   включая попадание максимума в последнюю корзину (нужен
   `least(widthBucket(…), n)`).
